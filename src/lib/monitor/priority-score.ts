 ...
 * 统一优先级评分系统
 *
 * 将关键词得分、信号加成、来源权重等多维度因素，归一化为 0-100 的统一优先级分数。
 *
 * ## 分数构成（满分 100 分）
 *
 * | 维度 | 满分 | 说明 |
 ...
 * | 来源权重 | 10 | 重点来源（Suno、Spotify、Apple Music 等）的动态天然权重更高 |
 ...
 * @property departmentName - 发布单位/来源名称
 ...
 */
import { FileText, Flame, Pin, Star, type LucideIcon } from "lucide-react";
 ...
 * 优先级分数的明细构成
 *
 * 用于展示分数的各维度贡献，帮助用户理解"为什么这条动态是这个优先级"。
 *
 ...
 * @property sourceBonus - 来源权重加成（0 或 10）
 * @property qualityBonus - 内容质量加成（满分 5）
 * @property total - 总分（关键词 + 信号 + 来源 + 质量）
 */
 ...
 */
const SIGNAL_BONUS = {
  /** 创作机会信号加成（10分）：比赛、驻留、资助等 */
  funding: 10,
  /** 申报截止预警加成（12分）：截止临近权重最高 */
  procurement: 12,
  /** AI工具更新加成（7分）：新功能、定价变化等 */
  pilot: 7,
  /** 平台政策/版权加成（6分）：规则变化等 */
  standards: 6,
};
 ...
 * 重点来源列表（加权）
 *
 * 这些来源发布的动态通常更具影响力，给予额外权重加成。
 * 包含：Suno、Spotify、Apple Music 等主流平台。
 */
 ...
 * 计算来源权重加成
 *
 * 重点来源（Suno、Spotify、Apple Music 等）发布的动态通常更具影响力，给予 10 分加成。
 *
 * @param departmentName - 发布单位/来源名称
 * @returns 来源权重加成（0 或 10）
 *
 * @example
 * ```ts
 * calculateSourceBonus("Suno");     // 10（重点来源）
 * calculateSourceBonus("某小众博客"); // 0（普通来源）
 * calculateSourceBonus("");          // 0（空）
 * ```
 *
 * @remarks
 * - 采用模糊匹配，包含关系即可命中（如"Spotify Newsroom"匹配"Spotify"）
 * - 重点来源包括：Suno、Spotify、Apple Music 等主流平台
 * - 这是一个二值判断（是/否），没有梯度
 */
export function calculateSourceBonus(departmentName?: string): number {
  if (!departmentName) {
    return 0;
  }
  const name = departmentName.trim();
  for (const dept of HIGH_WEIGHT_SOURCES) {
 ...
 * @param input.departmentName - 发布单位/来源名称
 ...
 * console.log(result.levelIcon);   // 等级图标组件
 ...
 *
 * ## 边界安全
 * - 普通动态（关键词 0-20 分）即使拉满所有加成，也不会跳到核心级
 * - 详见单元测试覆盖了各种边界场景
 *
...
export function calculatePriorityScore(input: PriorityScoreInput): PriorityScoreResult {
  ...
  const sourceBonus = calculateSourceBonus(input.departmentName);
  const qualityBonus = calculateQualityBonus(input);

  const total = Math.round(
    Math.min(
      TOTAL_MAX,
      keywordBase + signalBonus.total + sourceBonus + qualityBonus
    ) * 10
  ) / 10;
