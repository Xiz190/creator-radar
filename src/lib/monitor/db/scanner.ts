...
export async function scanAndApplyKeywords(sourceId: string, url: string): Promise<string[] | null> {
  ...
  // 正文太短通常只给到「重点内容」防标题党；但分数远超核心线（标题含 工具名+发布动词+版本 等多重发布信号）
  // 属实打实的重大发布，即便正文是 RSS 摘要也放行到「核心关注」。
  ...
  // 不再往 capture_note 里塞「得分明细」技术前缀（对创作者是噪音）；重要性改由 LLM 评级承担。
  // 仅保留原始抓取备注，并清掉历史遗留的得分前缀。
  const existingNoteRaw = row.capture_note ?? "";
  const newNote = existingNoteRaw.replace(/^【[^】]*得分\s*\d+[^】]*】\s*/, "").trim() || null;
...
// 音乐/创作者情报台的「内容体裁」——关键词与文章内容同步，避免全部落到"其他"
const GENRE_KEYWORDS: Record<string, string[]> = {
  发布更新: ["发布", "更新", "上线", "新功能", "新版本", "introducing", "launch", "release", "now available"],
  教程技巧: ["教程", "指南", "how to", "guide", "tutorial", "prompt", "技巧", "tips", "ways to"],
  评测榜单: ["评测", "对比", "review", "compared", "comparison", "best ", "top ", "榜单", "alternatives"],
  机会招募: ["招募", "征集", "比赛", "大赛", "open call", "submission", "报名", "festival", "扶持"],
  访谈播客: ["访谈", "专访", "interview", "podcast", "播客"],
  行业报告: ["报告", "趋势", "report", "trend", "statistics", "观察", "研究"],
  政策版权: ["版权", "政策", "policy", "copyright", "license", "授权", "侵权"],
};
