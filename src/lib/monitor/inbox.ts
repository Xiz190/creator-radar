...
import { getCachedCount, setCachedCount } from "@/lib/monitor/utils/cache";
import type { MonitorInboxItemRow } from "@/lib/monitor/db/items";
...
export async function getInboxItemsByFilter(params: {
  ...
  if (qNorm) {
    ...
    pb.addClause(`(
      ...
      or exists (select 1 from jsonb_array_elements_text(mi.content_json) as p where lower(p) like ${qPlaceholder})
    )`);
  }
  ...
  if (params.categories && params.categories.length > 0) {
    ...
    pb.addClause(
      `exists (select 1 from jsonb_array_elements(mi.matched_categories) as c where c->>'category' in ${pb.pushMany(expanded)})`,
    );
  }
  if (params.genres && params.genres.length > 0) {
    pb.addClause(
      `exists (select 1 from jsonb_array_elements_text(mi.matched_genres) as g where g in ${pb.pushMany(params.genres)})`,
    );
  }
  ...
  if (params.tags && params.tags.length > 0) {
    pb.addClause(
      `exists (select 1 from jsonb_array_elements(
        case when jsonb_typeof(mi.extracted_tags_json) = 'array' then mi.extracted_tags_json else '[]'::jsonb end
      ) as t where lower(t->>'tag') in ${pb.pushMany(params.tags.map(t => t.toLowerCase()))})`,
    );
  }

  if (params.tagCategories && params.tagCategories.length > 0) {
    pb.addClause(
      `exists (select 1 from jsonb_array_elements(
        case when jsonb_typeof(mi.extracted_tags_json) = 'array' then mi.extracted_tags_json else '[]'::jsonb end
      ) as t where t->>'category' in ${pb.pushMany(params.tagCategories)})`,
    );
  }

  if (params.excludeDuplicates) {
    pb.addClause(`mi.is_duplicate = false`);
  }
  ...
  if (params.subscribedKeywords && params.subscribedKeywords.length > 0) {
    const kwPlaceholders = params.subscribedKeywords.map((kw) => pb.push(`%${kw.toLowerCase()}%`)).join(" or ");
    subscriptionParts.push(`(lower(mi.title) like ${kwPlaceholders})`);
  }
  ...
  const limitVal = params.limit ?? 200;
  ...
  const res = await pool.query<{
      ...
      `select mi.source_id,
       ...
       limit ${Number(limitVal) || 200} offset ${Number(offsetVal) || 0}`,
      finalParams,
    );
