...
import { backfillMissingLens } from "@/lib/monitor/backfill-lens";
import {
  mockDelay,
  generateMockItems,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "backfill-lens") {
      const limit = Number(body?.limit ?? 40);
      const pool = getPgPool();
      const stats = await backfillMissingLens(pool, Number.isFinite(limit) && limit > 0 ? limit : 40);
      apiLog.success(startTime, "POST", request.url, 200, { action, filled: stats.filled, remaining: stats.remaining });
      return NextResponse.json({ ...stats, action: "backfill-lens" });
    }
  ...
  try {
    ...
    if (view === "debug") {
      ...
      const testRes = await pool.query(
        `select mi.source_id,
                coalesce(ms.department_name, '未分类机构') as department_name,
                coalesce(ms.channel_name, coalesce(ms.display_name, mi.source_id)) as channel_name,
                coalesce(ms.display_name, concat(coalesce(ms.department_name,'未分类机构'),'·',coalesce(ms.channel_name,''))) as display_name,
                mi.url,
                mi.title,
    ...
    const hasLens = url.searchParams.get("hasLens") === "1";
