...
describe("inbox-page-utils - GENRE_LIST 常量", () => {
  it("包含 8 个创作者内容体裁", () => {
    expect(GENRE_LIST.length).toBe(8);
  });

  it("是创作者/音乐域体裁（与 scanner 的 GENRE_KEYWORDS 一致），不是旧政务公文类型", () => {
    expect(GENRE_LIST).toContain("发布更新");
    expect(GENRE_LIST).toContain("机会招募");
    expect(GENRE_LIST).toContain("政策版权");
    expect(GENRE_LIST).toContain("其他");
  });
