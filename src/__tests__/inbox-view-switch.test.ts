...
describe("视图切换 - 基础常量", () => {
  ...
  it("SIGNAL_VIEW_CATEGORIES 包含 5 个信号分类", () => {
    ...
    expect(SIGNAL_VIEW_CATEGORIES).toContain("A·AI工具更新");
    expect(SIGNAL_VIEW_CATEGORIES).toContain("B·创作机会");
    expect(SIGNAL_VIEW_CATEGORIES).toContain("C·申报截止预警");
    expect(SIGNAL_VIEW_CATEGORIES).toContain("D·行业观察");
    expect(SIGNAL_VIEW_CATEGORIES).toContain("平台政策/版权");
  });
});
...
describe("getViewPresetState - signals（有信号）", () => {
  ...
  it("selectedCategories 包含全部 5 个信号分类", () => {
    ...
    expect(preset.selectedCategories).toContain("A·AI工具更新");
    expect(preset.selectedCategories).toContain("B·创作机会");
    expect(preset.selectedCategories).toContain("C·申报截止预警");
    expect(preset.selectedCategories).toContain("D·行业观察");
    expect(preset.selectedCategories).toContain("平台政策/版权");
  });
