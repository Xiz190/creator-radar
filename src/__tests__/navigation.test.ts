...
import { Bell, BookOpen, House, Inbox } from "lucide-react";
import {
  isNavActiveFn,
  isMenuDivider,
  primaryNav,
...
describe("导航 - settingsMenu（设置菜单配置）", () => {
  ...
  it("「数据洞察」分组有 2 个菜单项：高价值信号、数据趋势", () => {
    const groupItems = settingsMenu.slice(1, 3);
  ...
  it("第 4 个条目是「管理后台」分组分隔线", () => {
    const divider = settingsMenu[3];
  ...
  it("「管理后台」分组有 1 个菜单项：系统管理（后台能力统一入口）", () => {
    const groupItems = settingsMenu.slice(4, 5);
  ...
  it("第 6 个条目是「设置与帮助」分组分隔线", () => {
    const divider = settingsMenu[5];
    expect(isMenuDivider(divider)).toBe(true);
    if (isMenuDivider(divider)) {
...
describe("导航 - 信息架构验证", () => {
  ...
  it("设置菜单分三组：数据洞察（2项）+ 管理后台（1项）+ 设置与帮助（9项）", () => {
    ...
    expect(insightItems.length).toBe(2);
