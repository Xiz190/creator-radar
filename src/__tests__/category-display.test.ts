...
describe("分类展示映射 - 显示函数", () => {
  it("categoryDisplayLabel 返回用户友好的展示名", () => {
    expect(categoryDisplayLabel("A·AI工具更新")).toBe("AI工具更新");
    expect(categoryDisplayLabel("B·创作机会")).toBe("创作机会");
    expect(categoryDisplayLabel("C·申报截止预警")).toBe("截止预警");
    expect(categoryDisplayLabel("D·行业观察")).toBe("行业观察");
    expect(categoryDisplayLabel("AI音乐生成工具")).toBe("AI音乐工具");
  });

  it("categoryTooltip 返回说明文案", () => {
    const tooltip = categoryTooltip("B·创作机会");
    expect(tooltip).toContain("驻留");
...
describe("分类分组", () => {
  it("CATEGORY_GROUP_LABELS 覆盖全部 5 种分组", () => {
    ...
    expect(CATEGORY_GROUP_LABELS.signal).toBe("动态信号");
  ...
  it("信号层分类（5 个）都归为 signal 组", () => {
    const signalCats = ["A·AI工具更新", "B·创作机会", "C·申报截止预警", "D·行业观察", "平台政策/版权"];
    for (const cat of signalCats) {
      expect(getCategoryGroup(cat), `${cat} 应为 signal 组`).toBe("signal");
      expect(categoryGroupLabel(cat), `${cat} 应显示为政策信号`).toBe("动态信号");
  ...
  it("区域分类归为 region 组", () => {
    expect(getCategoryGroup("地方/省级/区域")).toBe("region");
    expect(categoryGroupLabel("地方/省级/区域")).toBe("区域相关");
    expect(REGION_CATEGORIES.has("地方/省级/区域")).toBe(true);
  });
...
describe("旧英文 key 向后兼容", () => {
  it("新分类体系英文 key 能正确映射", () => {
    expect(getCategoryStyle("signal_exec").displayLabel).toBe("AI工具更新");
    expect(getCategoryStyle("signal_support").displayLabel).toBe("创作机会");
    expect(getCategoryStyle("signal_risk").displayLabel).toBe("截止预警");
    expect(getCategoryStyle("topic_music").displayLabel).toBe("AI音乐工具");
    expect(getCategoryStyle("topic_site").displayLabel).toBe("发行平台");
    expect(getCategoryStyle("negative").displayLabel).toBe("其他");
  });

  it("旧分类体系英文 key 能正确映射", () => {
    expect(getCategoryStyle("ai").displayLabel).toBe("AI音乐工具");
    expect(getCategoryStyle("data").displayLabel).toBe("行业观察");
    expect(getCategoryStyle("industry").displayLabel).toBe("行业观察");
    expect(getCategoryStyle("security").displayLabel).toBe("截止预警");
  });

  it("连字符变体 key 能正确映射", () => {
    expect(getCategoryStyle("topic-ai").displayLabel).toBe("AI音乐工具");
    expect(getCategoryStyle("signal-risk").displayLabel).toBe("截止预警");
  });
});
...
describe("分类辅助函数", () => {
  it("getFirstTopicCategory 返回第一个主题分类的名称", () => {
    const cats = [
      { category: "C·申报截止预警", score: 5 },
      { category: "AI音乐生成工具", score: 10 },
      { category: "流媒体/发行平台", score: 8 },
    ];
    expect(getFirstTopicCategory(cats)).toBe("AI音乐生成工具");
  });

  it("getFirstTopicCategory 忽略信号分类和噪音", () => {
    const cats = [
      { category: "A·AI工具更新", score: 5 },
      { category: "B·创作机会", score: 3 },
  ...
  it("categoryMeta 返回 displayLabel 和颜色", () => {
    const meta = categoryMeta("B·创作机会");
    expect(meta.label).toBe("创作机会");
  ...
  it("filterTopicCategories 过滤出主题分类且保持顺序", () => {
    ...
    expect(filtered[0].category).toBe("AI音乐生成工具");
    expect(filtered[1].category).toBe("流媒体/发行平台");
  });
});
...
describe("向后兼容：legacyMap 完整性", () => {
  ...
  it("legacyMap 中所有 key 都能映射到有效的分类", () => {
    const testKeys = [
      ...
      "topic_music", "topic_craft", "topic_site", "topic_festival", "topic_education", "topic_unesco",
  ...
  it("连字符变体和下划线变体映射到同一个分类", () => {
    expect(getCategoryStyle("signal_exec").label).toBe(getCategoryStyle("signal-exec").label);
    expect(getCategoryStyle("signal_risk").label).toBe(getCategoryStyle("signal-risk").label);
  });

  it("旧 key 'ai' 和新 key 'topic_ai' 都映射到同一个分类", () => {
    expect(getCategoryStyle("ai").label).toBe(getCategoryStyle("topic_music").label);
  });
});
