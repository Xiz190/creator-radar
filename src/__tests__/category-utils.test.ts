...
describe("分类工具 - isSignalCategory（判断是否为信号层分类）", () => {
  it("A·AI工具更新 是信号层", () => {
    expect(isSignalCategory("A·AI工具更新")).toBe(true);
  });

  it("B·创作机会 是信号层", () => {
    expect(isSignalCategory("B·创作机会")).toBe(true);
  });

  it("C·申报截止预警 是信号层", () => {
    expect(isSignalCategory("C·申报截止预警")).toBe(true);
  });

  it("D·行业观察 是信号层", () => {
    expect(isSignalCategory("D·行业观察")).toBe(true);
  });

  it("平台政策/版权 是信号层", () => {
    expect(isSignalCategory("平台政策/版权")).toBe(true);
  });

  it("AI音乐生成工具 不是信号层", () => {
    expect(isSignalCategory("AI音乐生成工具")).toBe(false);
  });

  it("视频/视觉AI工具 不是信号层", () => {
    expect(isSignalCategory("视频/视觉AI工具")).toBe(false);
  });
  ...
  it("SIGNAL_CATEGORIES 集合包含全部 5 种信号层分类", () => {
    ...
    expect(SIGNAL_CATEGORIES.has("A·AI工具更新")).toBe(true);
    expect(SIGNAL_CATEGORIES.has("B·创作机会")).toBe(true);
    expect(SIGNAL_CATEGORIES.has("C·申报截止预警")).toBe(true);
    expect(SIGNAL_CATEGORIES.has("D·行业观察")).toBe(true);
    expect(SIGNAL_CATEGORIES.has("平台政策/版权")).toBe(true);
  });
});

describe("分类工具 - isTopicCategory（判断是否为主题层分类）", () => {
  it("AI音乐生成工具 是主题层", () => {
    expect(isTopicCategory("AI音乐生成工具")).toBe(true);
  });

  it("视频/视觉AI工具 是主题层", () => {
    expect(isTopicCategory("视频/视觉AI工具")).toBe(true);
  });

  it("流媒体/发行平台 是主题层", () => {
    expect(isTopicCategory("流媒体/发行平台")).toBe(true);
  });

  it("音乐比赛/节庆 是主题层", () => {
    expect(isTopicCategory("音乐比赛/节庆")).toBe(true);
  });

  it("版权/法律 是主题层", () => {
    expect(isTopicCategory("版权/法律")).toBe(true);
  });

  it("海外市场/国际 是主题层", () => {
    expect(isTopicCategory("海外市场/国际")).toBe(true);
  });

  it("A·AI工具更新 不是主题层", () => {
    expect(isTopicCategory("A·AI工具更新")).toBe(false);
  });

  it("B·创作机会 不是主题层", () => {
    expect(isTopicCategory("B·创作机会")).toBe(false);
  });
  ...
  it("地方/省级/区域 不是主题层（区域类不计入主题层用于匹配）", () => {
    expect(isTopicCategory("地方/省级/区域")).toBe(false);
  });
  ...
  it("TOPIC_CATEGORIES 集合包含全部 6 种主题层分类", () => {
    ...
    expect(TOPIC_CATEGORIES.has("AI音乐生成工具")).toBe(true);
    expect(TOPIC_CATEGORIES.has("视频/视觉AI工具")).toBe(true);
    expect(TOPIC_CATEGORIES.has("流媒体/发行平台")).toBe(true);
    expect(TOPIC_CATEGORIES.has("音乐比赛/节庆")).toBe(true);
    expect(TOPIC_CATEGORIES.has("版权/法律")).toBe(true);
    expect(TOPIC_CATEGORIES.has("海外市场/国际")).toBe(true);
  });
...
describe("分类工具 - filterTopicCategories（过滤主题层分类）", () => {
  it("全信号层输入返回空数组", () => {
    const input = [
      { category: "A·AI工具更新", score: 30 },
      { category: "B·创作机会", score: 28 },
      { category: "C·申报截止预警", score: 15 },
    ];
  ...
  it("全主题层输入返回全部", () => {
    const input = [
      { category: "AI音乐生成工具", score: 45 },
      { category: "视频/视觉AI工具", score: 38 },
      { category: "流媒体/发行平台", score: 25 },
    ];
    const result = filterTopicCategories(input);
  ...
  it("混合分类（主题在前）正确过滤出主题层（顺序无关）", () => {
    const input = [
      { category: "流媒体/发行平台", score: 42 },
      { category: "版权/法律", score: 35 },
      { category: "B·创作机会", score: 30 },
      { category: "A·AI工具更新", score: 25 },
      { category: "平台政策/版权", score: 18 },
    ];
    const result = filterTopicCategories(input);
  ...
  it("单一主题层分类返回包含该分类的数组", () => {
    const input = [{ category: "音乐比赛/节庆", score: 40 }];
    const result = filterTopicCategories(input);
    expect(result.length).toBe(1);
    expect(result[0].category).toBe("音乐比赛/节庆");
  });
  ...
  it("未知分类不计入主题层", () => {
    ...
    const result = filterTopicCategories(input);
    expect(result.length).toBe(1);
    expect(result[0].category).toBe("AI音乐生成工具");
  });
});
...
describe("分类工具 - getFirstTopicCategory（获取第一个主题层分类）", () => {
  ...
  it("主题层在前的混合分类返回第一个主题层", () => {
    ...
    const result = getFirstTopicCategory(input);
    expect(result).toBe("流媒体/发行平台");
  });

  it("全信号层分类返回 null（无主题层时 fallback）", () => {
    const input = [
      { category: "A·AI工具更新", score: 25 },
      { category: "B·创作机会", score: 20 },
      { category: "平台政策/版权", score: 15 },
      { category: "噪音词汇", score: 10 },
    ];
    const result = getFirstTopicCategory(input);
  ...
  it("单一主题层分类返回该分类", () => {
    const input = [{ category: "海外市场/国际", score: 30 }];
    const result = getFirstTopicCategory(input);
    expect(result).toBe("海外市场/国际");
  });

  it("噪音词汇在前时跳过噪音取第一个主题层", () => {
    const input = [
      { category: "噪音词汇", score: 10 },
      { category: "音乐比赛/节庆", score: 40 },
      { category: "B·创作机会", score: 25 },
    ];
    const result = getFirstTopicCategory(input);
    expect(result).toBe("音乐比赛/节庆");
  });
