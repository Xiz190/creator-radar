...
import {
  BookOpen, Glasses, Search, Star, Trash2,
} from "lucide-react";
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        ...
        >
          <div className="text-13 font-semibold text-slate-900"><Trash2 className="mr-1 inline h-3.5 w-3.5" aria-hidden />删除 90 天前条目</div>
          <div className="mt-1 text-xs text-slate-500">删除早期内容，释放数据库空间</div>
        </button>

        <button
          type="button"
          onClick={() => runAction("backfill-lens", { limit: 40 })}
          disabled={busy !== null}
          className="flex flex-col items-start rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-slate-400 hover:bg-slate-50 disabled:opacity-50"
        >
          <div className="text-13 font-semibold text-slate-900"><Glasses className="mr-1 inline h-3.5 w-3.5" aria-hidden />补齐创作者视角</div>
          <div className="mt-1 text-xs text-slate-500">给缺视角的条目生成（每次最多 40 条，只补空的、不覆盖已有）</div>
        </button>
