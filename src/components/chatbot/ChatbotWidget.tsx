...
import {
  Bot, ClipboardList, Coins, Download, FileText, FolderOpen, Lightbulb, Mailbox, MessageCircle, MessageSquare, Mic, Paperclip, Pin, Target, Trash2,
} from "lucide-react";
...
function generateFileResult(file: File, result: ParsedResult): string {
  ...
  if (result.text && result.text.length > 0) {
    lines.push(" 提示：已自动识别机会信号，点击下方结果可查看分析。");
...
function generateAnalysisResponse(fileName: string, analysis: SignalAnalysisResult): string {
  ...
  lines.push("");
  if (analysis.hasAnySignal) {
    lines.push("检测到以下机会信号：");
    if (analysis.hasFunding) lines.push("• 资金/资助：检测到资助、奖金、基金等关键词");
    if (analysis.hasProcurement) lines.push("• 征集/投递：检测到征集、招募、投稿等关键词");
    if (analysis.hasPilot) lines.push("• 演出/曝光：检测到演出、巡演、音乐节等关键词");
    if (analysis.hasStandards) lines.push("• 平台/版权：检测到版权、分成、平台政策等关键词");
    lines.push("");
    lines.push("匹配的关键词：" + (analysis.matchedKeywords?.slice(0, 10).join("、") || "—"));
  } else {
    lines.push(" 未检测到明显的机会信号。");
    lines.push("该文件可能是普通文档，或内容不含创作/机会相关关键词。");
  }
  return lines.join("\n");
}
