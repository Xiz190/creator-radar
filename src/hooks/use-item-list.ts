...
export type ContentItem = {
  ...
  /** 创作者视角的英文版。桌面站按界面语言二选一，伴侣版固定用它。 */
  creatorLensEn?: string | null;
  /** 用户自己写的备注（伴侣版 Saved 屏）。没写过是 null。 */
    ...
    try {
      ...
      const list: ContentItem[] = (json.items || []).map((raw) => {
        ...
        return {
          ...
          displayNameEn: (r.displayNameEn as string | undefined) || null,
          url: r.url as string,
          ...
          userNote: (r.userNote as string | undefined) || null,
        };
      });
      setItems(list);
